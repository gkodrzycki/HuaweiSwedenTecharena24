...
class SemiSupervisedLoss(torch.nn.Module):
    def __init__(self, margin=1.0, weight_unsupervised=0.1):
        super(SemiSupervisedLoss, self).__init__()
        self.margin = margin
        self.weight_unsupervised = weight_unsupervised

    def forward(self, y_close, y_anchor, y_far, y_true_anchor):
        """
        Args:
            y_close: Embedding of close points (positive samples).
            y_anchor: Embedding of anchor points.
            y_far: Embedding of far points (negative samples).
            y_true_anchor: Ground truth positions for anchors (optional).
        Returns:
            Loss value (float).
        """
        # Supervised Triplet Loss
        positive_distance = F.pairwise_distance(y_anchor, y_close, p=2)
        negative_distance = F.pairwise_distance(y_anchor, y_far, p=2)
        supervised_loss = F.relu(positive_distance - negative_distance + self.margin).mean()

        # Unsupervised Consistency Penalty
        unsupervised_loss = torch.mean((y_true_anchor - y_anchor) ** 2)

        # Combined Loss
        total_loss = supervised_loss + self.weight_unsupervised * unsupervised_loss
        return total_loss
